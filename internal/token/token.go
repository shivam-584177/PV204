...
type Token struct {
	...
	Algo         string `json:"algo"`          // e.g. "ECDSA-secp256k1-SHA256"
	SigB64       string `json:"sig_b64"`       // base64(signature bytes)
	PubKeyB64    string `json:"pubkey_b64"`    // base64(public key bytes)
}
...
// SigningBytes returns deterministic bytes to sign:
// SHA-256 of canonical JSON of token fields excluding signature and public key.
// Public key is excluded because it is only attached after the signing result returns.
